...
$backupFile = Join-Path $appDataDir 'acc-assist.settings.json.bak-diag2'
$exePath = Join-Path $env:LOCALAPPDATA 'Programs\acc-assist\ACCAssist.exe'
$stderrLog = Join-Path $appDataDir 'diag-stderr.log'
...
# Same settings as diagnose-tunnel.ps1
$sshSettings = @{
  gemini = @{
...
Write-Host '[2] Starting ACC Assist with stderr capture...' -NoNewline
$proc = Start-Process -FilePath $exePath -ArgumentList '--agent-debug-server-only' -RedirectStandardError $stderrLog -PassThru -NoNewWindow
...
Write-Host '[4] Waiting 20s for auto-connect...' -NoNewline
...
if (Test-Path $stderrLog) {
  ...
  $diagLines = $stderrContent | Where-Object { $_ -match 'DIAG|ESOCKET|error|Error|tunnel|forwardOut|stream|socket' }

...
$backupFile = Join-Path $appDataDir 'acc-assist.settings.json.bak-s18'
$exePath = Join-Path $env:LOCALAPPDATA 'Programs\acc-assist\ACCAssist.exe'

Write-Host '=== FIELD TEST S18 — Python Sandbox Regression ===' -ForegroundColor Cyan
...
$sshSettings = @{
  ...
  connectionProfiles = @(
    @{
      ...
      metadata = @{
        name = 'Sepidar SSH Field Test S18'
        description = 'SSH tunnel to Sepidar for S18 field test'
        type = 'ssh'
...
Write-Host ''
Write-Host '[5/6] Running test questions (regression + Python output prompts)...' -ForegroundColor Cyan
Write-Host ''

# Questions: mix of regression (existing metrics) + Python output prompts
# Python output prompts won't trigger PythonOutputPlan yet (S18.8 deferred)
# but should still return valid text results without errors
$questions = @(
  ...
  # 3 Python output prompts — should return text (no Python triggered yet)
...
# ── 7. Summary + cleanup ─────────────────────────────────────────────────────
Write-Host ''
Write-Host '=== FIELD TEST S18 RESULTS ===' -ForegroundColor Cyan
...
$exportPath = Join-Path $appDataDir 'field-test-s18-results.json'
...
if (Test-Path $backupFile) {
  ...
  Write-Host '[6/6b] Settings restored from backup.' -ForegroundColor Green
} else {
  Write-Host '[6/6b] No backup to restore — settings left as-is.' -ForegroundColor Yellow
}

Write-Host ''
if ($okCount -eq $results.Count) {
  Write-Host 'VERDICT: PASS — All questions answered correctly. No Python sandbox regression.' -ForegroundColor Green
} elseif ($okCount -ge ($results.Count - 1)) {
  Write-Host "VERDICT: PASS (with 1 acceptable failure) — No significant regression." -ForegroundColor Green
} else {

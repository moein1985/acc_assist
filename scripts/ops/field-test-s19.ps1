...
$backupFile = Join-Path $appDataDir 'acc-assist.settings.json.bak-s19'
$exePath = Join-Path $env:LOCALAPPDATA 'Programs\acc-assist\ACCAssist.exe'

Write-Host '=== FIELD TEST S19 — Advanced Financial Metrics ===' -ForegroundColor Cyan
  ...
  connectionProfiles = @(
    @{
      ...
      metadata = @{
        ...
        description = 'SSH tunnel to Sepidar for S19 field test'
        type = 'ssh'
...
Write-Host ''
Write-Host '[5/6] Running S19 test questions...' -ForegroundColor Cyan
Write-Host ''
...
$exportPath = Join-Path $appDataDir 'field-test-s19-results.json'
...
if ($okCount -eq $results.Count) {
  Write-Host 'VERDICT: PASS — All S19 questions answered correctly via SSH tunnel.' -ForegroundColor Green
